...
<template>
    ...
    >
        <div @click="$emit('open-modal', center)" class="flex items-center gap-4 sm:block group/content">
            <div
                class="w-28 h-28 shrink-0 sm:w-full sm:h-auto sm:aspect-[4/5] overflow-hidden sm:mb-6 bg-rose-50 rounded-3xl sm:rounded-[2rem] image-container shadow-sm group-hover:shadow-2xl transition-all duration-700 relative"
            >
                <img
                    class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-1000"
                    :src="center.image || '/images/beauty-wellness.avif'"
                    :alt="center.name"
                    loading="lazy"
                />
            ...
            <div class="flex-1 min-w-0 py-2 sm:py-0 px-0 sm:px-2 flex flex-col justify-center">
                <div class="flex justify-between items-start mb-1 sm:mb-2">
                    <h3 class="text-lg sm:text-2xl font-bold text-gray-900 leading-tight group-hover:text-rose-600 transition-colors duration-300 truncate w-full">
                        {{ center.name }}
                    </h3>
                </div>
